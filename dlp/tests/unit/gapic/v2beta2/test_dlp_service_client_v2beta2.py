...
class TestDlpServiceClient(object):
    def test_inspect_content(self):
        ...
        client = dlp_v2beta2.DlpServiceClient(channel=channel)
    ...
    def test_inspect_data_source(self):
        # Setup Expected Response
        name = 'name3373707'
        ...
        assert len(channel.requests) == 1
        expected_request = dlp_pb2.InspectDataSourceRequest(parent=parent)
        actual_request = channel.requests[0][1]
        assert expected_request == actual_request

    def test_inspect_data_source_exception(self):
        # Mock the API response
        channel = ChannelStub(responses=[CustomException()])
        client = dlp_v2beta2.DlpServiceClient(channel=channel)
        ...
        with pytest.raises(CustomException):
            client.inspect_data_source(parent)
    ...
    def test_analyze_data_source_risk(self):
        ...
        assert len(channel.requests) == 1
        expected_request = dlp_pb2.AnalyzeDataSourceRiskRequest(parent=parent)
        actual_request = channel.requests[0][1]
        assert expected_request == actual_request
    ...
    def test_analyze_data_source_risk_exception(self):
        ...
        with pytest.raises(CustomException):
            client.analyze_data_source_risk(parent)
    ...
    def test_update_inspect_template(self):
        ...
        # Setup Request
        name = client.inspect_template_path('[ORGANIZATION]',
                                            '[INSPECT_TEMPLATE]')
    ...
    def test_update_deidentify_template(self):
        ...
        # Setup Request
        name = client.deidentify_template_path('[ORGANIZATION]',
                                               '[DEIDENTIFY_TEMPLATE]')
